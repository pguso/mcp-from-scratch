// client.js - handshake, then call tools/list and print what the server exposes
//
...
async function handshake() {
  ...
  try {
    await sendUnchecked('tools/list', {});
    console.log('tools/list before init → unexpected success');
  } catch (err) {
    console.log('tools/list before init → rejected (expected):', err.message);
  }
  ...
  console.log('  tools capability:', JSON.stringify(initResult.capabilities?.tools));
...
function printTools(result) {
  ...
  console.log(`\n[client] tools/list → ${tools.length} tool(s)\n`);
